...
case "$ENVIRONMENT" in
  prod)
    ...
    ;;
  dev)
    DEPLOY_ENV_FILE="$ROOT_DIR/.env.deploy.dev"
    ;;
  *)
...
: "${DEPLOY_HOST:?Set DEPLOY_HOST in the selected deploy env file}"
: "${DEPLOY_USER:?Set DEPLOY_USER in the selected deploy env file}"
: "${DEPLOY_APP_DIR:?Set DEPLOY_APP_DIR in the selected deploy env file}"
...
if ! command -v ssh >/dev/null 2>&1; then
  echo "Error: ssh is required for deploy." >&2
  exit 1
fi
